...
import numpy as np
print("cd")
print("cd Compression_Energy_Experiment/compressors/zfp/examples/")
print("make all")
	...
	for file in dataSetName:
		if file is not dataSetPath:
			...
			for freq in np.arange(CPUMin, CPUMax, CPUStep):
				...
				print("perf stat -o data.txt -e power/energy-pkg/ /users/gfwilki/Compression_Energy_Experiment/compressors/zfp/examples/simple > cloud.f32.zfp")
				print("rm cloud.f32.zfp")
				print("grep -E \'Joules|time\' ./data.txt >> " + curRecFile)
				print("perf record /users/gfwilki/Compression_Energy_Experiment/compressors/zfp/examples/simple")
				print("rm cloud.f32.zfp")
